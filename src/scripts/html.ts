 ...
 */
function registerOnNumberChange(
  handlebar_template: HandlebarsTemplateDelegate<any>,
 ...
 */
function initIterationTemplate(context_selector: string, handlebar_selector: string, destination_selector: string) {
  // grab the source
  const source = (document.querySelector(handlebar_selector) as HTMLScriptElement).innerHTML;
  // compile it using Handlebars
  const template = Handlebars.compile(source);

  // context for Handlebar times iterator/ helper
  const input = document.getElementById(context_selector) as HTMLInputElement;
  ...
  // change destination
  (document.querySelector(destination_selector) as HTMLDivElement).innerHTML = template(context);

  // register Listeners
  registerOnNumberChange(template, document.querySelector(destination_selector) as HTMLDivElement, input);
}
